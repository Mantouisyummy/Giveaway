...
class Giveaway:
    ...
    async def start(self, bot:Bot, message_id:int, channel_id:int ,item:str, text:str, winners:int, timestamp:int, total:int):
    ...
    async def reroll(self, bot:Bot, message_id:int):
        ...
        components = [
            Button(label="參加",custom_id=f"giveaway_join_{channel.id}",style=ButtonStyle.green, emoji="🎉", disabled=True),
            Button(label="查看名單",custom_id=f"giveaway_list_{channel.id}",style=ButtonStyle.gray)
        ]
        ...
        if join_list != []:
            ...
            embed = Embed(title="🎊 重新抽獎! 🎊",description=f"{winner_string} 中了 {data['item']}！",colour=Colour.green())
            embed.set_footer(text="• 重新選擇中獎者請使用/giveaway reroll")
            msg_embed = Embed(title="🎊 重新抽獎! 🎊",description=f"中獎者: {winner_string}",colour=Colour.random())
            ...
            msg_embed = Embed(title="🎊 重新抽獎! 🎊",description=f"中獎者: 無",colour=Colour.random())
    ...
    async def end(self, bot:Bot, message_id:int, winners:int):
        ...
        if join_list != []:
            ...
            embed = Embed(title="沒有人參加本次的抽獎!",colour=Colour.red())
            msg_embed = Embed(title="🎊 抽獎已結束! 🎊",description=f"中獎者: 無",colour=Colour.random())
            await channel.send(embed=embed)
            await msg.edit(embed=msg_embed, components=components)
    ...
    async def edit(self, interaction:MessageInteraction, message_id:int):
        ...
        join_list = data['join_list']
        text = data['text']
        embed = Embed(title=item,description=f"抽獎資訊：{text}\n獲獎人數：{winners}\n結束時間：<t:{int(timestamp)}:R>\n參加人數：{len(join_list)}",colour=Colour.random())
